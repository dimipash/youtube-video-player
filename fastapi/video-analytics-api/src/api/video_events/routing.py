...
def get_top_video_stats(request: Request, db_session: Session = Depends(get_session)):
    ...
    query = (
        ...
        .group_by(bucket)
        .order_by(bucket.desc(), unique_views.desc())
    )
    try:
...
def get_video_stats(
    ...
    query = (
        select(
            ...
            YouTubeWatchEvent.video_id,
            func.count().label("total_events"),
            func.max(YouTubeWatchEvent.current_time).label(
                "max_viewership"
            ),  # in seconds
            func.avg(YouTubeWatchEvent.current_time).label(
                "avg_viewership"
            ),  # in seconds
            func.count(func.distinct(YouTubeWatchEvent.watch_session_id)).label(
                "unique_views"
